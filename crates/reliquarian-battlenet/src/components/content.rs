...
impl Component for BattleNetContentElement
{
	...
	{
		use_init_radio_station::<BattleNetGames, GamePlatforms>(Default::default);
		
		let mut selectedGame = use_radio::<BattleNetGames, GamePlatforms>(GamePlatforms::BattleNet);
		
		let diabloElement: Option<Element> = match *selectedGame.read() == BattleNetGames::Diablo3
		{
			false => None,
			true => Some(d3Element().into()),
		};
		
		let starcraftElement: Option<Element> = match *selectedGame.read() == BattleNetGames::StarCraft2
		{
			false => None,
			true => Some(sc2Element().into()),
		};
		
		let worldOfWarcraftElement: Option<Element> = match *selectedGame.read() == BattleNetGames::WorldOfWarcraft
		{
			false => None,
			true => Some(wowElement().into()),
		};
		
		let worldOfWarcraftClassicElement: Option<Element> = match *selectedGame.read() == BattleNetGames::WorldOfWarcraftClassic
		{
			false => None,
			true => Some(wowcElement().into()),
		};
		
		let title = match *selectedGame.read()
		{
			BattleNetGames::None => "Battle.Net".to_string(),
			_ => selectedGame.read().as_ref().to_string(),
		};
								...
								rect()
									...
									.child(
										Button::new()
											...
											.on_press(move |_| **selectedGame.write() = BattleNetGames::Diablo3)
									)
									...
									.child(
										Button::new()
											.child("StarCraft II")
											.width(Size::flex(0.25))
											.on_press(move |_| **selectedGame.write() = BattleNetGames::StarCraft2)
									)
							)
								...
								rect()
									...
									.child(
										Button::new()
											...
											.width(Size::flex(0.25))
											.on_press(move |_| **selectedGame.write() = BattleNetGames::WorldOfWarcraft)
									)
									...
									.child(
										Button::new()
											...
											.width(Size::flex(0.25))
											.on_press(move |_| **selectedGame.write() = BattleNetGames::WorldOfWarcraftClassic)
									)
							)
