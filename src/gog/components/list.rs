...
impl Component for GameList
{
	fn render(&self) -> impl IntoElement
	{
		...
		let games = appData.read().user.gog.filterGames(search.read().clone());
...
impl Component for GameListNode
{
	...
	{
		let appData = use_radio::<AppData, AppDataChannel>(AppDataChannel::Gog);
		let mut selectedGameId = use_radio::<Option<u64>, GameIdChannel>(GameIdChannel::Gog);
		
		let mut hovering = use_state(|| false);
		
		let game = appData.read().user.gog.getGame(self.gameId)
			.unwrap_or_default();
